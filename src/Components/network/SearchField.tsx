...
interface SearchFieldProps {
  ...
  highlightNode: string | null; // 新增的屬性
} 
/**
 ...
 */
const SearchField: FC<SearchFieldProps> = ({ filters, highlightNode }) => {
  const sigma = useSigma();

  const [search, setSearch] = useState<string>("");
  const [values, setValues] = useState<Array<{ id: string; label: string }>>([]);
  const [selected, setSelected] = useState<string | null>(null);
  // const [highlightedNode, setHighlightedNode] = useState<string | null>(highlightNode); // 新增的狀態
  ...
  useEffect(() => {
    ...
    sigma.getGraph().setNodeAttribute(highlightNode, "highlighted", true);
    const nodeDisplayData = sigma.getNodeDisplayData(highlightNode);

    if (nodeDisplayData)
      sigma.getCamera().animate(
        { ...nodeDisplayData, ratio: 1 },
        {
          duration: 600,
        },
      );
    ...
    return () => {
      sigma.getGraph().setNodeAttribute(highlightNode, "highlighted", false);
    };
  }, [highlightNode]); // 新增的 useEffect

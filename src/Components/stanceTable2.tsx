...
const DataTableComponent: React.FC<MyDataTable3Props> = ({ data }) => {
    ...
    useEffect(() => {
        if (data && data['data'] && data['data']['new']) {
            const transformedData = Object.keys(data['data']['new']['created_at']).map((value) => ({
                created_at: data['data']['new'].created_at[value],
                text: data['data']['new'].text[value]
            }));
